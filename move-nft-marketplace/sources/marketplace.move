module new_addr::marketplace{
    ...
    struct OfferStore has key {
        offers: Table<fungible_asset::TokenId, Offer>
    }
    ...
    struct ListTokenEvent has drop, store {
        market_id: MarketId,
        token_id: fungible_asset::TokenId,
        seller: address,
        price: u64,
        timestamp: u64,
        offer_id: u64
    ...
    fun get_royalty_fee_rate(token_id: fungible_asset::TokenId) : u64{
        let royalty = fungible_asset::get_royalty(token_id);
        let royalty_denominator = fungible_asset::get_royalty_denominator(&royalty);
        let royalty_fee_rate = if (royalty_denominator == 0) {
            0
        } else {
            fungible_asset::get_royalty_numerator(&royalty) / fungible_asset::get_royalty_denominator(&royalty)
        };
        royalty_fee_rate
    }
    ...
    public entry fun create_market<CoinType>(sender: &signer, market_name: String, fee_numerator: u64, fee_payee: address, initial_fund: u64) acquires MarketEvents, Market {        
        ...
        if(!exists<Market>(sender_addr)){
            ...
            fungible_asset::initialize_token_store(&resource_signer);
    ...
    public entry fun list_token<CoinType>(seller: &signer, market_address:address, market_name: String, creator: address, collection: String, name: String, property_version: u64, price: u64) acquires MarketEvents, Market, OfferStore {
        ...
        let token_id = fungible_asset::create_token_id_raw(creator, collection, name, property_version);
        let fungible_asset = fungible_asset::withdraw_token(seller, token_id, 1);

        fungible_asset::deposit_token(&resource_signer, fungible_asset);
